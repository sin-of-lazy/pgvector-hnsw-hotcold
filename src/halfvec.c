/*
 * halfvec.c - 16位半精度向量（halfvec）类型的完整实现
 *
 * 与 vector.c 结构完全对应，实现了 halfvec 类型的所有功能：
 *
 * 类型 I/O：
 *   halfvec_in/out - 文本格式（与 vector 相同，如 "[1.0,2.0]"）
 *   halfvec_recv/send - 二进制格式
 *   InitHalfVector - 分配零初始化的半精度向量
 *
 * 距离函数：
 *   halfvec_l2_distance/halfvec_inner_product 等
 *   调用 halfutils.c 中的硬件加速函数（F16C SIMD）
 *
 * 特殊处理：
 *   - half 的 NaN/Inf 检测（通过 HalfIsNan/HalfIsInf 宏）
 *   - float32 → half 转换的溢出检查（超出 HALF_MAX 时报错）
 *   - 半精度的输出需要升精度为 float 再格式化
 *
 * 与 vector.c 的差异：
 *   - 元素类型为 half（16位），存储密度是 float32 的 2 倍
 *   - 精度较低（约 3 位十进制），适用于嵌入向量、推荐系统等场景
 *   - 最大维度同为 16000
 */
#include "postgres.h"
