/*
 * ivfflat.c - IVFFlat 索引的 PostgreSQL 访问方法注册与初始化
 *
 * 与 hnsw.c 对应，本文件负责：
 *   1. 将 IVFFlat 注册为 PostgreSQL 的索引访问方法
 *   2. 注册 GUC 参数（ivfflat.probes 等）
 *   3. 注册索引存储选项（lists 参数）
 *   4. 实现代价估算（ivfflatcostestimate）
 *   5. 返回 IndexAmRoutine 结构体
 *
 * IVFFlat 的代价模型：
 *   访问 probes 个列表，每个列表平均包含 n/lists 个向量，
 *   代价约为 O(probes * n/lists) 的线性扫描代价。
 */
#include "postgres.h"
 ...
 */
/*
 * IvfflatInit() - 初始化 IVFFlat 索引的参数和选项
 *
 * 输入：无
 * 输出：无
 *
 * 注册内容：
 *   - lists：聚类中心数，作为索引 reloption 存储（CREATE INDEX WITH 选项）
 *   - ivfflat.probes：查询时探测的列表数，GUC 参数（会话级可调）
 *   - ivfflat.iterative_scan：迭代扫描模式，GUC 枚举参数
 *   - ivfflat.max_probes：迭代扫描的最大探测数
 */
void
IvfflatInit(void)
{
