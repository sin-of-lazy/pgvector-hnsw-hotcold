/*
 * hnswvacuum.c - HNSW 索引的 VACUUM（空间回收）实现
 *
 * HNSW 的 VACUUM 流程：
 *   1. hnswbulkdelete：标记阶段
 *      - 扫描所有索引页，找出 heap 中已删除行对应的索引条目
 *      - 调用 callback(tid) 判断是否应删除（由 VACUUM 命令提供）
 *      - 在元素元组中设置 deleted=1 标记（软删除，不立即释放页）
 *      - 收集被删除元素的 TID，供 hnswvacuumcleanup 使用
 *
 *   2. hnswvacuumcleanup：清理阶段
 *      - 遍历所有邻居元组，从邻居列表中移除已标记删除的节点
 *      - 若入口点被删除，寻找新的入口点
 *      - 更新统计信息（死亡元组数、页数等）
 *
 * 与 HNSW 图的兼容性：
 *   直接删除节点会破坏图的连通性，导致某些节点变得不可达。
 *   pgvector 采用"软删除"策略：先标记，再清理邻居引用，
 *   确保删除后图仍然可导航。
 */
#include "postgres.h"
 ...
 */
/*
 * hnswbulkdelete(info, stats, callback, callback_state) - 批量删除回调
 *
 * 输入：
 *   info           - VACUUM 信息（索引、快照等）
 *   stats          - 统计结果（输入可为 NULL，会自动分配）
 *   callback       - 判断 TID 是否应删除的回调函数
 *   callback_state - 传给 callback 的额外状态
 * 输出：IndexBulkDeleteResult（包含删除的元组数等统计）
 *
 * 扫描所有索引页，对每个元素元组检查其 heap TID 是否被删除，
 * 若是则设置 deleted 标记（不修改邻居关系，由 vacuumcleanup 处理）。
 */
 ...
 */
/*
 * hnswvacuumcleanup(info, stats) - VACUUM 清理阶段回调
 *
 * 在 bulkdelete 之后执行，负责：
 *   1. 从邻居元组中移除已标记删除节点的引用
 *   2. 若入口点被删除，通过遍历图找新入口点（取最高层的存活节点）
 *   3. 更新索引统计信息（用于查询优化器的代价估算）
 */
IndexBulkDeleteResult *
