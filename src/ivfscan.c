/*
 * ivfscan.c - IVFFlat 索引的查询扫描实现
 *
 * IVFFlat 扫描流程：
 *   1. ivfflatbeginscan  - 分配 IvfflatScanOpaqueData
 *   2. ivfflatrescan     - 初始化查询：
 *      - 计算查询向量与所有质心的距离
 *      - 用优先队列选出最近的 probes 个质心
 *      - 将这 probes 个列表的 startPage 存入 listPages
 *   3. ivfflatgettuple   - 每次返回一个结果：
 *      - 扫描当前列表的所有向量，加入 tuplesort（按距离排序）
 *      - 从 sort 中依次取出并返回（保证结果按距离有序）
 *   4. ivfflatendscan    - 清理资源
 *
 * 迭代式扫描：
 *   类似 HNSW，当 probes 不够时自动增加探测列表数。
 *
 * 距离函数路由：
 *   so->distfunc 在 rescan 时根据算子类选择正确的距离计算函数，
 *   支持 vector/halfvec 的 L2/余弦/内积/L1 等多种距离。
 */
#include "postgres.h"
 ...
 */
/*
 * ivfflatgettuple(scan, dir) - 返回下一个 IVFFlat 扫描结果
 *
 * 输入：scan - 扫描描述符，dir - 方向（仅支持前向）
 * 输出：true=有结果（scan->xs_heaptid 已设置），false=扫描结束
 *
 * 首次调用时：
 *   - 扫描 probes 个选定列表的所有向量页
 *   - 对每个向量计算与查询向量的距离
 *   - 插入 tuplesort 按距离排序
 * 后续调用：从 tuplesort 依次取出已排序的结果。
 */
bool
