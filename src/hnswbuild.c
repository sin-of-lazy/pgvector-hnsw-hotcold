/*
 * hnswbuild.c - HNSW 索引构建（BUILD）的完整实现
 *
 * 构建流程：
 *   1. hnswbuild() - 入口，初始化构建状态，决定是否并行构建
 *   2. 单进程构建（BuildGraph）：
 *      - 扫描 heap 中的所有行（table_index_build_scan）
 *      - 对每一行调用 InsertTupleOnDisk，将其插入 HNSW 图
 *   3. 并行构建（ParallelBuildGraph）：
 *      - 启动多个 worker 进程（背后使用 PG 并行框架）
 *      - 各 worker 读取 heap 的不同分区
 *      - 通过共享内存（HnswShared/HnswGraph）同步图状态
 *      - leader 进程最后将内存图写入磁盘页
 *
 * 磁盘写入（FlushGraph）：
 *   将内存中构建好的 HNSW 图按 PostgreSQL 的页格式写入索引文件，
 *   包括元数据页（metapage）、元素元组页和邻居元组页。
 *
 * HNSW 图插入算法（简要）：
 *   1. 随机确定新元素的最高层级 l（概率衰减，由 ml 控制）
 *   2. 从入口点开始，在 l+1 到最高层逐层贪心搜索最近的入口
 *   3. 在 0 到 l 层，用 SearchLayer 找 ef_construction 个最近候选
 *   4. 用 SelectNeighbors 从候选中选 m 个（或 2m 个用于底层）建立连接
 *   5. 双向连接：也更新被选邻居的连接列表（可能触发缩减）
 */
/*
 ...
 */
/*
 * hnswbuild(heap, index, indexInfo) - HNSW 索引构建的 PG 回调入口
 *
 * 输入：
 *   heap      - 数据表的 Relation
 *   index     - 正在构建的索引 Relation
 *   indexInfo - 索引定义信息（列、表达式、条件等）
 * 输出：IndexBuildResult（包含 indtuples 统计信息）
 *
 * PG 在 CREATE INDEX 时调用此函数。
 * 根据 max_parallel_maintenance_workers 决定是否并行构建。
 */
IndexBuildResult *
 ...
 */
/*
 * hnswbuildempty(index) - 为空索引创建初始元数据页
 *
 * 在 CREATE INDEX ... (无数据) 或 REINDEX 前清空时调用。
 * 写入元数据页（block 0），标记索引为空（无入口点）。
 * 使用 INIT_FORKNUM fork 写入，允许在空表上创建索引。
 */
void
