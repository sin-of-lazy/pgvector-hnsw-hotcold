/*
 * hnsw.c - HNSW 索引的 PostgreSQL 访问方法注册与初始化
 *
 * 本文件负责：
 *   1. 将 HNSW 注册为 PostgreSQL 的索引访问方法（Index Access Method）
 *   2. 定义和注册 HNSW 的 GUC 参数（hnsw.ef_search 等）
 *   3. 定义索引选项（m、ef_construction 存储为 reloption）
 *   4. 实现代价估算回调（hnswcostestimate）
 *   5. 暴露 IndexAmRoutine 结构体给 PostgreSQL
 *
 * PostgreSQL 索引访问方法（Index AM）集成原理：
 *   CREATE INDEX ... USING hnsw 时，PG 通过 pg_am 系统表找到
 *   注册的 handler 函数（hnswhandler），该函数返回一个
 *   IndexAmRoutine 结构体，其中包含所有操作的函数指针：
 *     ambuild     → hnswbuild     (构建索引)
 *     aminsert    → hnswinsert    (插入元素)
 *     ambeginscan → hnswbeginscan (开始扫描)
 *     amgettuple  → hnswgettuple  (获取下一个结果)
 *     ambulkdelete→ hnswbulkdelete(批量删除)
 *   等等。
 */
#include "postgres.h"
 ...
 */
/*
 * HnswInitLockTranche() - 为 HNSW 的 LWLock 注册 tranche ID
 *
 * 输入：无
 * 输出：无（设置全局 hnsw_lock_tranche_id）
 *
 * LWLock（Lightweight Lock）是 PostgreSQL 的轻量级锁，用于保护
 * 共享内存数据结构的并发访问。每种锁类型需要一个 tranche ID。
 * ShmemInitStruct：在共享内存中分配或查找命名结构体，
 * found=true 表示已由其他进程初始化（只需读取），否则需要初始化。
 * AddinShmemInitLock：PG 提供的保护共享内存初始化的专用锁。
 */
void
 ...
 */
/*
 * HnswInit() - 初始化 HNSW 索引的参数和选项
 *
 * 输入：无
 * 输出：无
 *
 * 注册以下内容：
 * 1. add_reloption_kind：为 HNSW 创建专属的 reloption 命名空间
 * 2. add_int_reloption：注册 m 和 ef_construction 为索引存储选项
 *    （在 CREATE INDEX ... WITH (m=16) 中使用，持久化到 pg_class）
 * 3. DefineCustomIntVariable / DefineCustomEnumVariable：注册 GUC 参数
 *    （如 SET hnsw.ef_search = 100，为会话级参数，不持久化）
 *
 * process_shared_preload_libraries_in_progress：
 *   若在 shared_preload_libraries 中加载，PG 已初始化共享内存，
 *   可直接调用 HnswInitLockTranche；否则延迟到首次使用时初始化。
 */
void
HnswInit(void)
{
 ...
 */
/*
 * hnswcostestimate - HNSW 索引扫描的代价估算回调
 *
 * 输入：
 *   root          - 查询优化器的 PlannerInfo
 *   path          - 候选的索引路径
 *   loop_count    - 外层循环次数（嵌套循环连接时使用）
 * 输出（指针参数）：
 *   indexStartupCost - 启动代价（第一行结果前的代价）
 *   indexTotalCost   - 总代价
 *   indexSelectivity - 选择率（估计返回行比例）
 *   indexCorrelation - 物理顺序相关性（HNSW 结果无序，设为 0）
 *   indexPages       - 估计访问的页数
 *
 * 若查询没有 ORDER BY（无向量排序），则设为无穷大代价拒绝使用索引。
 * HNSW 只对 ORDER BY 查询有意义（KNN 搜索）。
 * 代价模型：基于 m 参数估算每次查询大约访问 m * ef_search 个节点。
 */
static void
