/*
 * ivfbuild.c - IVFFlat 索引构建的完整实现
 *
 * 构建分为三个阶段（进度报告中体现）：
 *
 * 阶段 1 - K-means 聚类（PROGRESS_IVFFLAT_PHASE_KMEANS）：
 *   - 从 heap 中随机采样（reservoir sampling，水塘抽样）
 *   - 调用 IvfflatKmeans 对样本进行 k-means 聚类
 *   - 得到 lists 个聚类中心（质心向量）
 *
 * 阶段 2 - 向量分配（PROGRESS_IVFFLAT_PHASE_ASSIGN）：
 *   - 再次扫描 heap，为每个向量找最近质心
 *   - 将 (质心ID, 向量, TID) 插入临时排序结构（tuplesort）
 *   - 并行构建时各 worker 分别处理，结果合并
 *
 * 阶段 3 - 写入磁盘（PROGRESS_IVFFLAT_PHASE_LOAD）：
 *   - 按质心ID排序，依次将向量写入对应倒排列表的页
 *   - 写入质心向量到列表头页（IvfflatListData）
 *
 * 采样策略（水塘抽样）：
 *   不预知数据量，在 O(n) 时间内均匀采样，
 *   确保 k-means 质心能代表全量数据分布。
 */
#include "postgres.h"
 ...
 */
/*
 * ivfflatbuild(heap, index, indexInfo) - IVFFlat 索引构建入口
 *
 * 输入：heap - 数据表，index - 索引，indexInfo - 索引定义
 * 输出：IndexBuildResult（含 indtuples 统计）
 *
 * 执行完整的三阶段构建流程，并在最后更新索引元数据页。
 */
IndexBuildResult *
