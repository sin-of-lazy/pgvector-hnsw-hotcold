/*
 * hnswutils.c - HNSW 算法核心工具函数
 *
 * 本文件包含 HNSW 图操作的核心逻辑，是整个 HNSW 实现的基础：
 *
 * 图构建核心：
 *   HnswSearchLayer     - 在指定层搜索最近邻候选集（greedy best-first search）
 *   HnswFindElementNeighbors - 为新节点找邻居并建立双向连接
 *   HnswUpdateConnection - 更新邻居数组（含超出 m 时的缩减）
 *
 * 磁盘 I/O：
 *   HnswInsertTupleOnDisk   - 将新元素写入索引页
 *   HnswUpdateNeighborsOnDisk - 更新邻居元组（写入 WAL 保证持久化）
 *   HnswLoadElement         - 从磁盘页加载元素到内存
 *   HnswLoadNeighborTids    - 加载邻居 TID 列表
 *
 * 内存管理：
 *   HnswAlloc               - 通过自定义分配器分配内存（支持共享内存分配）
 *   HnswInitElement         - 初始化一个新的图节点
 *   HnswInitNeighborArray   - 初始化某层的邻居数组
 *
 * 元数据：
 *   HnswGetMetaPageInfo     - 读取索引元数据（入口点、参数等）
 *   HnswUpdateMetaPage      - 更新元数据（写入 WAL）
 *
 * 搜索算法（HnswSearchLayer）：
 *   基于优先队列的贪心最优优先搜索：
 *   - 维护候选集 C（按距离升序）和结果集 W（按距离降序）
 *   - 每次从 C 中取最近的节点，若比 W 中最远的更近则加入 W
 *   - 对其邻居计算距离，更新 C 和 W
 *   - 直到 C 的最近候选比 W 的最远结果还远（无法改善）
 */
#include "postgres.h"
 ...
 */
/*
 * HnswGetEntryPoint(index) - 获取 HNSW 图的当前入口点
 *
 * 输入：HNSW 索引 Relation
 * 输出：入口点的 HnswElement（若图为空则返回 NULL）
 *
 * 入口点是 HNSW 图中层级最高的节点，所有搜索从此开始。
 * 通过 LockPage(HNSW_SCAN_LOCK) 保护并发读取。
 */
HnswElement
 ...
 */
/*
 * HnswSearchLayer - HNSW 图中单层的近似最近邻搜索
 *
 * 输入：
 *   base      - 共享内存基地址（并行构建时），NULL 表示普通内存
 *   q         - 查询向量
 *   ep        - 入口点列表（从上层搜索结果传入）
 *   ef        - 动态候选集大小（ef_construction 或 ef_search）
 *   lc        - 当前搜索层级
 *   index     - HNSW 索引 Relation（NULL 时使用内存图）
 *   support   - 距离函数等支持函数
 *   m         - 每层最大连接数
 *   inserting - 是否处于插入模式（影响访问集的数据结构选择）
 *   skipElement - 插入时跳过自身（避免自连接）
 *   v         - 已访问节点集合（passed in，函数内更新）
 *   discarded - 被丢弃的候选者堆（用于迭代式扫描）
 *   initVisited - 是否初始化 v
 *   tuples    - 已访问 tuple 计数器
 * 输出：List* 结果候选列表（ef 个最近邻，按距离排序）
 *
 * 使用 pairingheap（配对堆）实现优先队列，O(log n) 插入/删除。
 * visited_hash 避免重复访问同一节点（关键优化，防止无限循环）。
 */
List *
