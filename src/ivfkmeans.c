/*
 * ivfkmeans.c - IVFFlat 使用的 K-means 聚类算法实现
 *
 * 实现了 Lloyd's K-means 算法（带 k-means++ 初始化）：
 *
 * 初始化（k-means++）：
 *   1. 随机选第一个中心
 *   2. 之后每次以 D(x)^2 概率选新中心（D(x) 为到最近中心的距离）
 *   3. 距离现有中心越远的点越可能被选为下一个中心
 *   目的：减少收敛迭代次数，避免糟糕的随机初始化
 *
 * 迭代（Lloyd's 算法）：
 *   1. 分配：将每个样本分配到最近的质心
 *   2. 更新：重新计算每个簇的均值作为新质心
 *   3. 重复直到收敛（质心变化 < 阈值）或达到最大迭代次数
 *
 * 支持不同向量类型：
 *   通过 IvfflatTypeInfo 的 updateCenter/sumCenter 回调适配
 *   vector/halfvec/sparsevec 的不同内存布局。
 *
 * 球形 K-means（余弦距离）：
 *   每次更新后对质心做 L2 归一化，使其保持在单位超球面上。
 */
#include "postgres.h"
 ...
 */
/*
 * IvfflatKmeans(index, samples, centers, typeInfo) - 执行 K-means 聚类
 *
 * 输入：
 *   index    - IVFFlat 索引（用于读取 lists 数和距离函数）
 *   samples  - 采样的向量集合（VectorArray）
 *   typeInfo - 类型操作接口（含 updateCenter/sumCenter）
 * 输出：centers（原地更新，包含聚类后的 lists 个质心）
 *
 * 若样本数 < lists，直接复制样本作为质心（不需要聚类）。
 * 内部实现 k-means++ 初始化 + Lloyd 迭代，最多 100 次迭代。
 */
void
