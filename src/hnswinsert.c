/*
 * hnswinsert.c - HNSW 索引的单行插入实现
 *
 * 当 INSERT/UPDATE 修改数据行时，PG 调用 hnswinsert 将新向量插入索引。
 *
 * 插入流程（与构建期类似，但需要更严格的并发控制）：
 *   1. 从值构造向量 Datum（处理 NULL、归一化等）
 *   2. 在索引上加 HNSW_UPDATE_LOCK 写锁（防止并发图结构修改）
 *   3. 调用 HnswInsertTupleOnDisk：
 *      a. 找到当前入口点
 *      b. 确定新元素的层级（HnswGetMl 控制）
 *      c. 用 HnswSearchLayer 找各层最近邻
 *      d. 在磁盘页上写入元素元组和邻居元组
 *      e. 更新受影响邻居的连接（可能缩减）
 *      f. 若新元素层级更高，更新元数据中的入口点
 *   4. 释放锁
 *
 * 并发安全性：
 *   每个元素有独立的 LWLock（element->lock），
 *   修改邻居关系时需持有 element lock 和 index lock。
 */
#include "postgres.h"
 ...
 */
/*
 * hnswinsert(index, values, isnull, heap_tid, ...) - 索引插入回调
 *
 * 输入：
 *   index     - HNSW 索引 Relation
 *   values    - 要索引的列值数组（向量）
 *   isnull    - 对应列是否为 NULL
 *   heap_tid  - 新行的 heap TID（物理位置）
 *   heap      - 数据表 Relation
 *   checkUnique - 唯一性检查（HNSW 不支持，始终为 UNIQUE_CHECK_NO）
 * 输出：bool（HNSW 不支持唯一性，始终返回 false）
 *
 * NULL 向量不被索引（直接返回 false）。
 * indexUnchanged（PG 14+）：HOT 更新时若索引列未变，可跳过索引更新。
 */
bool
