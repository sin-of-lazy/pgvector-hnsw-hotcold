/*
 * ivfutils.c - IVFFlat 索引的工具函数
 *
 * 提供 IVFFlat 索引页管理和列表更新的工具函数：
 *   - 页的初始化、分配、提交（IvfflatInitPage/NewBuffer/AppendPage）
 *   - 列表元数据更新（IvfflatUpdateList）
 *   - 类型信息获取（IvfflatGetTypeInfo）
 *   - 归一化（IvfflatNormValue）
 *
 * 页管理与 WAL：
 *   IVFFlat 使用 generic_xlog（通用 WAL）记录页修改，
 *   确保崩溃恢复后索引数据一致性。
 *   HNSW 使用 smgrwrite（绕过 WAL），适合构建期批量写入。
 */
#include "postgres.h"
