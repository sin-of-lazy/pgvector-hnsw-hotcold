/*
 * sparsevec.c - 稀疏向量（sparsevec）类型的完整实现
 *
 * 本文件实现了 sparsevec 类型的所有 PostgreSQL 函数：
 *
 * 类型 I/O：
 *   sparsevec_in/out  - 文本格式："{dim:[idx:val,...]}n/dim" 格式
 *   sparsevec_recv/send - 二进制格式（网络字节序）
 *   sparsevec_typmod_in - 解析类型修饰符
 *
 * 距离函数：
 *   sparsevec_l2_distance/sparsevec_inner_product 等
 *   针对稀疏向量的优化：只遍历非零元素，O(nnz) 而非 O(dim)
 *
 * 类型转换：
 *   vector_to_sparsevec / sparsevec_to_vector / array_to_sparsevec 等
 *
 * 算术运算：
 *   sparsevec_add/sub/mul 等（结果也是稀疏向量）
 *
 * 文本格式说明：
 *   例：{3:[0:1.5,2:3.0]}/5 表示 5 维向量，第 0 维=1.5，第 2 维=3.0
 *   格式：{nnz:[idx:val,...]/dim（0-based 索引）
 *
 * 稀疏向量距离计算的关键优化：
 *   两个稀疏向量的点积只需遍历共有非零索引（归并两个有序列表），
 *   时间复杂度 O(nnz_a + nnz_b)，远小于稠密向量的 O(dim)。
 */
#include "postgres.h"
